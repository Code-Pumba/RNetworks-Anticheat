import { rlife_base_is_production } from '../global';
...
export class LogConsoleHandler implements LogHandler {
    ...
    public constructor() {
        if (rlife_base_is_production) {
            this.level = LogLevel.Info;
        }
    }

    private format(color: LogColor, ...message: string[]): string {
        return `^${color.valueOf().toString()}[${new Date().toLocaleTimeString()}] [rNetworks-Anticheat]: ${message.join(' ')}^7`;
    }
